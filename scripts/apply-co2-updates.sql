-- Apply CO2 calculation updates to FoodVrse database
-- Run this in your Supabase SQL Editor: https://supabase.com/dashboard/project/vsvhkkalfziuyttwityc/sql

-- Update the trigger function with new CO2 calculation (2.5 kg per meal)
...
BEGIN
  ...
  IF NEW.status = 'collected' AND (OLD.status IS NULL OR OLD.status != 'collected') THEN
    ...
    -- Real-world CO2 calculation: 2.5 kg CO2 per meal saved
