-- Add user_type enum to differentiate between consumer and business accounts
...
-- Add user_type column to business_profiles  
ALTER TABLE public.business_profiles 
ADD COLUMN user_type public.user_type DEFAULT 'business'::public.user_type;

-- Create unique constraints to ensure a user can only have one profile type
ALTER TABLE public.user_profiles 
...
-- Update RLS policies to enforce user type separation
...
USING (auth.uid() = user_id AND user_type = 'consumer');

CREATE POLICY "Consumers can insert their own profile" 
ON public.user_profiles FOR INSERT 
WITH CHECK (auth.uid() = user_id AND user_type = 'consumer');
...
CREATE POLICY "Business users can create their own business profile" 
ON public.business_profiles FOR INSERT 
WITH CHECK (auth.uid() = user_id AND user_type = 'business');

CREATE POLICY "Business users can update their own business profile" 
ON public.business_profiles FOR UPDATE 
USING (auth.uid() = user_id AND user_type = 'business');

-- Create view for easy user type checking
